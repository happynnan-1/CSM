...
namespace DataAccess
{
    internal interface IDapperImplementor
    {
        ...
        //肖艳杰添加,增加分页方法
        IEnumerable<T> GetPage<T>(IDbConnection connection, string sql, int page, int resultsPerPage, object parameters, IDbTransaction transaction, int? commandTimeout, bool buffered) where T : class;
    }
    ...
    internal class DapperImplementor : IDapperImplementor
    {
        ...
        //肖艳杰添加,增加基于SQL语句的分页方法
        public IEnumerable<T> GetPage<T>(IDbConnection connection, string sql, int page, int resultsPerPage,
            object parameters, IDbTransaction transaction, int? commandTimeout, bool buffered) where T : class
        {
            object pageParameters;
            var newSql = SqlGenerator.SelectPaged(sql, page, resultsPerPage, out pageParameters);
            var dynamicParameters = parameters == null ? new DynamicParameters() : new DynamicParameters(parameters);
            dynamicParameters.AddDynamicParams(pageParameters);

            return connection.Query<T>(newSql, dynamicParameters, transaction, buffered, commandTimeout, CommandType.Text);
        }
