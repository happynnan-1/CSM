...
namespace DataAccess
{
    internal interface ISqlDialect
    {
        ...
        //肖艳杰添加,对基于动态类型的查询添加方法
        string GetPagingSql(string sql, int page, int resultsPerPage,out object parameters);
        string GetSetSql(string sql, int firstResult, int maxResults, out object parameters);
    ...
    internal abstract class SqlDialectBase : ISqlDialect
    {
        ...
        //肖艳杰添加,对基于动态类型的查询添加查询方法
        public abstract string GetPagingSql(string sql, int page, int resultsPerPage,out object parameters);
        public abstract string GetSetSql(string sql, int firstResult, int maxResults,out object parameters);
        public virtual bool IsQuoted(string value)
        {
            //肖艳杰添加,如果为*则不需要添加"[]",直接返回true
            var newValue = value.Trim();
            if (newValue == "*")
                return true;
            if (newValue[0] == OpenQuote)
            {
